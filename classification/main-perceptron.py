...
class PerceptronClassifier():
  ...
  def train(self):
    ...
    for epoch in range(self.num_epochs):
      ...
      print 'accuracy in training set=',self.accuracy_trn, 'of the ', epoch+1, 'th epoch'
      self.accuracy_dev.append(self.compute_DevelopmentSet_accuracy(w))
      print 'accuracy in development set=',self.accuracy_dev, 'of the ', epoch+1, 'th epoch'
    self.weights=w
    return w
  ...
  def compute_DevelopmentSet_accuracy(self,w):
    correct = 0.0
    # Bag_DevSet = bag_of_words_all(self.developSet_X, len(self.developSet_X), self.vocabulary_dict)
  ...
  def test(self,tst_x_file):
    ...
    fileObject = open('perceptron-test.pred', 'w')
...
PC=PerceptronClassifier(1.0,6,'trn.data','trn.label','dev.data','dev.label')
t0 = time()
weights=PC.train()
print '\n'
print 'training is over!'
print 'Trained W:', weights
print 'Error rate on training set', 1.0-PC.accuracy_trn[-1]
print 'Error rate on development set', 1.0-PC.accuracy_dev[-1]
print 'training time is : ', time()-t0

fig=plt.figure(1)
fig1=fig.add_subplot(111)
fig1.set_title('Perceptron Training Iteration')
fig1.set_xlabel("Epochs")
plt.xlim(0,6.0)
fig1.set_ylabel("accuracies")
plt.ylim(0,1.5)
plot1 = fig1.plot(range(len(PC.accuracy_trn)), PC.accuracy_trn, '-r',label='accuracy curves on training set')
plot2 = fig1.plot(range(len(PC.accuracy_trn)), PC.accuracy_dev, '-b',label='accuracy curves on development set')
plt.legend(loc='upper right')
plt.show()
fig.savefig('Perceptron_Plot.jpg',dpi=fig.dpi)
